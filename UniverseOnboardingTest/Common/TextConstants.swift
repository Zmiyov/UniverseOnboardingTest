//
//  TextConstants.swift
//  UniverseOnboardingTest
//
//  Created by Volodymyr Pysarenko on 05.06.2024.
//

import Foundation

enum OnboardingScreenTxt {
    static let title = "Let’s setup App for you"
    static let buttonTitle = "Continue"
}

enum PaywallScreenTxt {
    static let title = "Discover all \nPremium features"
    static let buttonTitle = "Start Now"
    static let termsTitle = "By continuing you accept our: \nTerms of Use, Privacy Policy, Subscription Terms"
}

enum MainAppScreenTxt {
    static let subscribed = "Subscribsion is active"
    static let unSubscribed = "Subscribsion isn't active"
}

